...
# Два круглых нагревателя
num_heaters = 2

# Площадь одного круглого нагревателя (суммарная площадь двух = площади исходного)
single_area = orig_area / num_heaters  # 0.06 м²

# Радиус круглого нагревателя
heater_radius = np.sqrt(single_area / np.pi)  # ~0.138 м

# ============ КЛЮЧЕВЫЕ КОНСТАНТЫ ============
R = 0.25  # расстояние от центра стержня до центра каждого нагревателя, м

# Углы расположения нагревателей (в градусах)
# Для двух нагревателей: 90° и 270° (вверх и вниз) или 0° и 180° (влево-вправо)
angles_deg = [90, 270]  # можно менять!
angles_rad = np.deg2rad(angles_deg)

# Сетка (уменьшил для устойчивости)
Ny = 51
Nz = 51
Nt = 2000
...
print(f"Два круга: каждый площадью {single_area:.4f} м²")
...
plt.suptitle(f'Два круглых нагревателя (R={R} м, r={heater_radius:.3f} м, углы {angles_deg}°)')
plt.tight_layout()
plt.show()
...
plt.title(f'Два круглых нагревателя (R={R} м): изменение температуры')
...
plt.xlabel('y, м')
plt.ylabel('z, м')
plt.title(f'Два круглых нагревателя: зоны нагрева выше 310K\nНагрето {percent_heated:.1f}% площади вне нагревателей')
plt.show()
